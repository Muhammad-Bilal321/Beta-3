  ...
  return (
    <div className="m-2">
      ...
      {courseList.map((course, index) => (
        <div className="card text-center" key={index} style={{ width: "18rem" }} >
          <div className="card-body">
            <h5 className="card-title">{course.name}</h5>
            <h6 className="card-subtitle mb-2 text-muted"><strong>Teacher:</strong> {course.teacher}</h6>
            <p className="card-text"><strong>Fee:</strong> {course.fee}</p>
            <p className="card-text"><strong>Duration:</strong> {course.duration}</p>
<Button variant="contained" onClick={handleClick}>More Details</Button>
          </div>
        </div>
    )  )}
    </div>
